...
parser.add_argument('--time_slot', type = int, default = 5,help = 'The time step for Abilene and CERNET is 5 minutes and for GEANT is 15 minutes.')
parser.add_argument('--M', type = int, default = 12,help = 'history time steps')
parser.add_argument('--N', type = int, default = 12,help = 'prediction time steps')
...
parser.add_argument('--traffic_file', default = 'data/AbileneTM.h5',help = 'traffic file')
parser.add_argument('--SE_file', default = 'data/SE.txt',help = 'spatial emebdding file')
parser.add_argument('--model_file', default = 'data/ASTGN(Abilene)')
parser.add_argument('--log_file', default = 'data/log(Abilene)',help = 'log file')
...
num_train, _, Z = trainX.shape
X, TE, label, is_training = model.make_ph(args.M, args.N, Z)
...
pred = model.ASTGN(
    X, TE, SE, args.M, args.N, T, args.L, args.K, args.d,
...
for n in range(args.N):
    mae, rmse, mape = utils.metric(testPred[:, n], testY[:, n])
    MAE.append(mae)
    RMSE.append(rmse)
    MAPE.append(mape)
    utils.log_string(log, 'step: %02d         %.2f\t\t%.2f\t\t%.2f%%' %
                     (n + 1, mae, rmse, mape * 100))
